...
Pixel* convertToPixelArray(BMP bmp) {
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            bmp.pImageData += alpha;
            newPixel[index].B = *(bmp.pImageData)++;
            newPixel[index].G = *(bmp.pImageData)++;
            newPixel[index].R = *(bmp.pImageData)++;
        }
        bmp.pImageData += padding;
    }
...
void convertTo8Bit(BMP srcBMP, BMP& dstBMP, Pixel* srcPixel) {
    ...
    for (int y = 0; y < dstHeight; y++) {
        for (int x = 0; x < dstWidth; x++) {
            ...
            *(dstBMP.pImageData)++ = (srcPixel[index].B + srcPixel[index].G + srcPixel[index].R) / 3;
        }
        dstBMP.pImageData += dstPadding;
    }
}
...
void resize(BMP srcBMP, BMP& dstBMP, int scale) {
    ...
    if (dstBMP.dib.bitsPerPixel == 8) {
        ...
        for (int y = 0; y < dstHeight; y++) {
            for (int x = 0; x < dstWidth; x++) {
                int index = getIndex(dstWidth, x, y);
                *(dstBMP.pImageData)++ = dstArray[index];
            }
            dstBMP.pImageData += dstPadding;
        }
    }
    ...
    // ảnh là 24-bit/32-bit
        ...
        for (int y = 0; y < dstHeight; y++) {
            for (int x = 0; x < dstWidth; x++) {
                int index = getIndex(dstWidth, x, y);
                dstBMP.pImageData += alpha;
                *(dstBMP.pImageData)++ = dstPixel[index].B;
                *(dstBMP.pImageData)++ = dstPixel[index].G;
                *(dstBMP.pImageData)++ = dstPixel[index].R;
            }
            dstBMP.pImageData += dstPadding;
        }
        delete[] dstPixel;
